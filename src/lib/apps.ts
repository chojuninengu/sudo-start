    ...
    versions: [
      {
        ...
        linuxCommand: 'echo "🔍 Checking dependencies..." && for cmd in curl chmod find mkdir; do command -v "$cmd" >/dev/null 2>&1 || { echo "❌ Required dependency $cmd not found. Installing..."; sudo apt-get install -y "$cmd"; }; done && APPIMAGE_PATH=$(find "$HOME/Downloads" -type f \\( -name "Cursor-*.AppImage" -o -name "cursor-*.AppImage" \\) | sort | tail -n 1) && if [ -z "$APPIMAGE_PATH" ]; then echo "❌ Cursor AppImage not found in ~/Downloads. Please download it first from cursor.com"; exit 1; fi && APPIMAGE_NAME=$(basename "$APPIMAGE_PATH") && INSTALL_DIR="/opt/cursor" && DESKTOP_FILE="$HOME/.local/share/applications/cursor.desktop" && ICON_NAME="cursor.png" && ICON_URL="https://www.cursor.com/brand/icon.svg" && echo "📦 Installing Cursor from $APPIMAGE_NAME..." && sudo mkdir -p "$INSTALL_DIR" && sudo cp "$APPIMAGE_PATH" "$INSTALL_DIR/cursor" && sudo chmod +x "$INSTALL_DIR/cursor" && echo "🎨 Downloading official Cursor icon..." && mkdir -p "$HOME/.local/share/icons" && if curl -s -o "$HOME/.local/share/icons/cursor-icon.svg" "$ICON_URL"; then sudo cp "$HOME/.local/share/icons/cursor-icon.svg" "$INSTALL_DIR/$ICON_NAME" || sudo cp /usr/share/icons/hicolor/256x256/apps/application-default-icon.png "$INSTALL_DIR/$ICON_NAME"; else cd /tmp && "$APPIMAGE_PATH" --appimage-extract > /dev/null 2>&1 && if [ -f "squashfs-root/usr/share/pixmaps/co.anysphere.cursor.png" ]; then sudo cp "squashfs-root/usr/share/pixmaps/co.anysphere.cursor.png" "$INSTALL_DIR/$ICON_NAME"; else echo "⚠️ Icon not found. Using fallback icon."; sudo cp /usr/share/icons/hicolor/256x256/apps/application-default-icon.png "$INSTALL_DIR/$ICON_NAME"; fi && rm -rf squashfs-root; fi && sudo ln -sf "$INSTALL_DIR/cursor" /usr/local/bin/cursor && cat > "$DESKTOP_FILE" <<EOF && [Desktop Entry]\\nName=Cursor\\nExec=/opt/cursor/cursor\\nIcon=$INSTALL_DIR/$ICON_NAME\\nType=Application\\nCategories=Development;Utility;\\nTerminal=false\\nStartupWMClass=Cursor\\nComment=Cursor is an AI-first coding environment.\\nEOF && chmod +x "$DESKTOP_FILE" && update-desktop-database ~/.local/share/applications/ || true && echo "🔒 Configuring AppArmor profile (Ubuntu 24.04+)..." && if command -v apparmor_parser >/dev/null 2>&1; then echo "# AppArmor profile for Cursor\\nabi <abi/4.0>,\\ninclude <tunables/global>\\nprofile cursor $INSTALL_DIR/cursor flags=(unconfined) {\\n  userns,\\n  include if exists <local/cursor>\\n}" | sudo tee /etc/apparmor.d/cursor > /dev/null && sudo apparmor_parser -r /etc/apparmor.d/cursor 2>/dev/null || echo "⚠️ AppArmor configuration skipped (non-Ubuntu system)"; else echo "ℹ️ AppArmor not available - skipping security profile"; fi && echo "✅ Cursor installed successfully! You can now run it using: cursor"',
      },
      {
        id: 'download',
        label: 'Download Only',
