...
export function generateTerminalScript(os: OS, packages: Package[]): string {
    const terminalPackages = packages.filter((pkg) => pkg.category === 'terminal');
    if (terminalPackages.length === 0) {
        return '';
    }

    const lines: string[] = [];
    lines.push('echo ""');
    lines.push('echo "Installing Terminals..."');
    lines.push('echo ""');

    terminalPackages.forEach((pkg) => {
