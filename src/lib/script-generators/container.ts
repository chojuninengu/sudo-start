...
export function generateContainerScript(os: OS, packages: Package[]): string {
    const containerPackages = packages.filter((pkg) => pkg.category === 'container');
    ...
    const lines: string[] = [];
    lines.push('echo ""');
    lines.push('echo "Installing Containers..."');
    lines.push('echo ""');

    containerPackages.forEach((pkg) => {
