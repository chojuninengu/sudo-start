...
export function generateDatabaseScript(os: OS, packages: Package[]): string {
    const databasePackages = packages.filter((pkg) => pkg.category === 'database');
    ...
    const lines: string[] = [];
    lines.push('echo ""');
    lines.push('echo "Installing Databases..."');
    lines.push('echo ""');

    databasePackages.forEach((pkg) => {
